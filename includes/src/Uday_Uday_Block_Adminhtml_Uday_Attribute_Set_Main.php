...
class Uday_Uday_Block_Adminhtml_Uday_Attribute_Set_Main extends Mage_Adminhtml_Block_Template
{
    
    protected function _construct()
    {
        $this->setTemplate('uday/attribute/set/main.phtml');
    }
    ...
    protected function _prepareLayout()
    {
        ...
        $this->setChild('group_tree',
            $this->getLayout()->createBlock('uday/adminhtml_uday_attribute_set_main_tree_group')
        );

        $this->setChild('edit_set_form',
            $this->getLayout()->createBlock('uday/adminhtml_uday_attribute_set_main_formset')
        );

        $this->setChild('delete_group_button',
            $this->getLayout()->createBlock('adminhtml/widget_button')->setData(array(
                'label'     => Mage::helper('uday')->__('Delete Selected Group'),
        ...
        $this->setChild('add_group_button',
            $this->getLayout()->createBlock('adminhtml/widget_button')->setData(array(
                'label'     => Mage::helper('uday')->__('Add New'),
        ...
        $this->setChild('back_button',
            $this->getLayout()->createBlock('adminhtml/widget_button')->setData(array(
                'label'     => Mage::helper('uday')->__('Back'),
        ...
        $this->setChild('reset_button',
            $this->getLayout()->createBlock('adminhtml/widget_button')->setData(array(
                'label'     => Mage::helper('uday')->__('Reset'),
                'onclick'   => 'window.location.reload()'
        )));

        $this->setChild('save_button',
            $this->getLayout()->createBlock('adminhtml/widget_button')->setData(array(
                'label'     => Mage::helper('uday')->__('Save Attribute Set'),
                'onclick'   => 'editSet.save();',
                'class'     => 'save'
        )));

        $deleteConfirmMessage = $this->jsQuoteEscape(Mage::helper('uday')
            ->__('All products of this set will be deleted! Are you sure you want to delete this attribute set?'));
        $deleteUrl = $this->getUrl('*/adminhtml_uday_set/delete', array('id' => $setId));

        $this->setChild('delete_button',
            $this->getLayout()->createBlock('adminhtml/widget_button')->setData(array(
                'label'     => Mage::helper('uday')->__('Delete Attribute Set'),
        ...
        $this->setChild('rename_button',
            $this->getLayout()->createBlock('adminhtml/widget_button')->setData(array(
                'label'     => Mage::helper('uday')->__('New Set Name'),
    ...
    {
        return Mage::helper('uday')->__("Edit Attribute Set '%s'", $this->_getAttributeSet()->getAttributeSetName());
    }

    public function getMoveUrl()
    {
        return $this->getUrl('*/adminhtml_uday_set/save', array('id' => $this->_getSetId()));
    }

    public function getGroupUrl()
    {
        return $this->getUrl('*/adminhtml_uday_group/save', array('id' => $this->_getSetId()));
    }
    ...
    public function getGroupTreeJson()
    {
        ...
        foreach ($groups as $node) {
            ...
            $nodeChildren = Mage::getResourceModel('uday/uday_attribute_collection')
    ...
    public function getAttributeTreeJson()
    {
        ...
        $collection = Mage::getResourceModel('uday/uday_attribute_collection')
            ->setAttributeSetFilter($setId)
            ->load();
        ...
        $attributes = Mage::getResourceModel('uday/uday_attribute_collection')
        ...
        if (count($items) == 0) {
            $items[] = array(
                'text'      => Mage::helper('uday')->__('Empty'),
                'id'        => 'empty',
                'cls'       => 'folder',
                'allowDrop' => false,
                'allowDrag' => false,

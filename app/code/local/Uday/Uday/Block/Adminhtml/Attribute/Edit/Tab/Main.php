    ...
    extends Mage_Eav_Block_Adminhtml_Attribute_Edit_Main_Abstract
{
    protected function _prepareForm()
    {
        ...
        $additionalTypes = array(
            array(
                'value' => 'price',
                'label' => Mage::helper('uday')->__('Price')
            ),
            array(
                'value' => 'media_image',
                'label' => Mage::helper('uday')->__('Media Image')
            )
        );
        if ($attributeObject->getFrontendInput() == 'gallery') {
            $additionalTypes[] = array(
                'value' => 'gallery',
                'label' => Mage::helper('uday')->__('Gallery')
            );
        }
        ...
        $fieldset->addField('is_global', 'select', array(
            'name'  => 'is_global',
            'label' => Mage::helper('uday')->__('Scope'),
            'title' => Mage::helper('uday')->__('Scope'),
            'note'  => Mage::helper('uday')->__('Declare attribute value saving scope'),
            'values'=> $scopes
        ), 'attribute_code');
        ...
        $fieldset->addField('is_configurable', 'select', array(
            ...
            'label' => Mage::helper('uday')->__('Use To Create Configurable Product'),
            'values' => $yesnoSource,
        ), 'apply_to');

        // frontend properties fieldset
        $fieldset = $form->addFieldset('front_fieldset', array('legend'=>Mage::helper('uday')->__('Frontend Properties')));

        $fieldset->addField('is_searchable', 'select', array(
            'name'     => 'is_searchable',
            'label'    => Mage::helper('uday')->__('Use in Quick Search'),
            'title'    => Mage::helper('uday')->__('Use in Quick Search'),
            'values'   => $yesnoSource,
        ));

        $fieldset->addField('is_visible_in_advanced_search', 'select', array(
            'name' => 'is_visible_in_advanced_search',
            'label' => Mage::helper('uday')->__('Use in Advanced Search'),
            'title' => Mage::helper('uday')->__('Use in Advanced Search'),
            'values' => $yesnoSource,
        ));

        $fieldset->addField('is_comparable', 'select', array(
            'name' => 'is_comparable',
            'label' => Mage::helper('uday')->__('Comparable on Front-end'),
            'title' => Mage::helper('uday')->__('Comparable on Front-end'),
            'values' => $yesnoSource,
        ));

        $fieldset->addField('is_filterable', 'select', array(
            'name' => 'is_filterable',
            'label' => Mage::helper('uday')->__("Use In Layered Navigation"),
            'title' => Mage::helper('uday')->__('Can be used only with catalog input type Dropdown, Multiple Select and Price'),
            'note' => Mage::helper('uday')->__('Can be used only with catalog input type Dropdown, Multiple Select and Price'),
            'values' => array(
                array('value' => '0', 'label' => Mage::helper('uday')->__('No')),
                array('value' => '1', 'label' => Mage::helper('uday')->__('Filterable (with results)')),
                array('value' => '2', 'label' => Mage::helper('uday')->__('Filterable (no results)')),
            ),
        ));

        $fieldset->addField('is_filterable_in_search', 'select', array(
            'name' => 'is_filterable_in_search',
            'label' => Mage::helper('uday')->__("Use In Search Results Layered Navigation"),
            'title' => Mage::helper('uday')->__('Can be used only with catalog input type Dropdown, Multiple Select and Price'),
            'note' => Mage::helper('uday')->__('Can be used only with catalog input type Dropdown, Multiple Select and Price'),
            'values' => $yesnoSource,
        ));
        ...
        $fieldset->addField('is_used_for_promo_rules', 'select', array(
            ...
            'label' => Mage::helper('uday')->__('Use for Promo Rule Conditions'),
            'title' => Mage::helper('uday')->__('Use for Promo Rule Conditions'),
            'values' => $yesnoSource,
        ));

        $fieldset->addField('position', 'text', array(
            'name' => 'position',
            'label' => Mage::helper('uday')->__('Position'),
            'title' => Mage::helper('uday')->__('Position in Layered Navigation'),
            'note' => Mage::helper('uday')->__('Position of attribute in layered navigation block'),
            'class' => 'validate-digits',
        ));

        $fieldset->addField('is_wysiwyg_enabled', 'select', array(
            'name' => 'is_wysiwyg_enabled',
            'label' => Mage::helper('uday')->__('Enable WYSIWYG'),
            'title' => Mage::helper('uday')->__('Enable WYSIWYG'),
            'values' => $yesnoSource,
        ));

        $htmlAllowed = $fieldset->addField('is_html_allowed_on_front', 'select', array(
            'name' => 'is_html_allowed_on_front',
            'label' => Mage::helper('uday')->__('Allow HTML Tags on Frontend'),
            'title' => Mage::helper('uday')->__('Allow HTML Tags on Frontend'),
            'values' => $yesnoSource,
        ));
        ...
        $fieldset->addField('is_visible_on_front', 'select', array(
            ...
            'label'     => Mage::helper('uday')->__('Visible on Product View Page on Front-end'),
            'title'     => Mage::helper('uday')->__('Visible on Product View Page on Front-end'),
            'values'    => $yesnoSource,
        ));

        $fieldset->addField('used_in_product_listing', 'select', array(
            'name'      => 'used_in_product_listing',
            'label'     => Mage::helper('uday')->__('Used in Product Listing'),
            'title'     => Mage::helper('uday')->__('Used in Product Listing'),
            'note'      => Mage::helper('uday')->__('Depends on design theme'),
            'values'    => $yesnoSource,
        ));
        $fieldset->addField('used_for_sort_by', 'select', array(
            'name'      => 'used_for_sort_by',
            'label'     => Mage::helper('uday')->__('Used for Sorting in Product Listing'),
            'title'     => Mage::helper('uday')->__('Used for Sorting in Product Listing'),
            'note'      => Mage::helper('uday')->__('Depends on design theme'),
            'values'    => $yesnoSource,
        ));
